...
from data_collectors.bybit.spot.database import DatabaseManager
...
def calculate_gaps(db_manager: DatabaseManager, symbol: str, hours: int = 24) -> List[Tuple[str, str, int]]:
    ...
    query = """
        SELECT EXTRACT(EPOCH FROM timestamp)::bigint * 1000 as open_time
        FROM candles_bybit_spot_1m
        WHERE symbol = %s
        AND timestamp >= to_timestamp(%s::bigint / 1000)
        AND timestamp <= to_timestamp(%s::bigint / 1000)
        ORDER BY timestamp
    """
    ...
    try:
        ...
        query = """
            SELECT
                ...
                EXTRACT(EPOCH FROM MIN(timestamp))::bigint * 1000 as first_candle,
                EXTRACT(EPOCH FROM MAX(timestamp))::bigint * 1000 as last_candle,
                (EXTRACT(EPOCH FROM MAX(timestamp))::bigint - EXTRACT(EPOCH FROM MIN(timestamp))::bigint) * 1000 as time_range_ms
            FROM candles_bybit_spot_1m
            GROUP BY symbol
            ORDER BY symbol
        """
        ...
        with db_manager.get_connection() as conn:
            ...
            for row in rows:
                ...
                if delay_minutes > 5:
                    print(
                        f"  ⚠️  {symbol}: Данные устарели на {int(delay_minutes)} минут - запустите monitor_spot.py"
                    )
