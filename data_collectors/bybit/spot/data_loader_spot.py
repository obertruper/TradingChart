...
class SpotCollector:
    ...
    def _calculate_smart_chunks(self, start_timestamp: int, end_timestamp: int) -> List[Tuple[int, int]]:
        ...
        self.logger.debug(f"Created {len(chunks)} smart chunks for {expected_candles} expected candles")
        return chunks
    ...
    def _split_period_into_days(self, start_date: str, end_date: str) -> List[Tuple[str, str]]:
        """
        Split a collection period into daily batches for incremental DB writes.

        This prevents memory overflow by processing 1 day (1440 candles) at a time,
        with each day written to DB immediately after collection.

        Args:
            start_date: Start date string in format "YYYY-MM-DD HH:MM:SS"
            end_date: End date string in format "YYYY-MM-DD HH:MM:SS"

        Returns:
            List of (day_start_str, day_end_str) tuples, one per day
        """
        start_dt = self.time_manager.parse_datetime_string(start_date)
        end_dt = self.time_manager.parse_datetime_string(end_date)
        ...
        while current_day_start < end_dt:
            ...
            day_end = datetime.datetime(
                current_day_start.year,
                current_day_start.month,
            ...
            current_day_end = min(day_end, end_dt)

            # Add this day to the list
            daily_periods.append((
                current_day_start.strftime("%Y-%m-%d %H:%M:%S"),
                current_day_end.strftime("%Y-%m-%d %H:%M:%S")
            ))

            # Move to next day (start at 00:00:00)
            current_day_start = day_end + datetime.timedelta(seconds=1)
            if current_day_start.hour != 0 or current_day_start.minute != 0:
                current_day_start = current_day_start.replace(hour=0, minute=0, second=0)

        return daily_periods

    def collect_symbol_data(self, symbol: str, start_date: str, end_date: str, show_progress: bool = True) -> int:
        """
        ...
        Args:
            ...
            start_date: Start date string
            end_date: End date string
            show_progress: Show tqdm progress bar for chunks (default True)

        Returns:
            Number of candles collected
        """
        self.logger.debug(f"Starting data collection for {symbol} from {start_date} to {end_date}")
        ...
        # Create progress bar for chunks (only if show_progress=True)
        if show_progress:
            pbar = tqdm(
                time_ranges,
        ...
        if all_collected_candles:
            self.logger.debug(f"Filtering {len(all_collected_candles)} raw candles to exact time range")
            ...
            if filtered_candles:
                ...
                if unique_candles:
                    ...
                    self.logger.debug(f"No unique candles found for {symbol} after deduplication")
            else:
                self.logger.debug(f"No candles found for {symbol} in target time range")
        else:
            self.logger.debug(f"No candles fetched for {symbol}")

        self.logger.debug(f"Completed {symbol}: {candles_collected} candles collected")
    ...
    def _collect_symbol_smart(self, symbol: str, missing_periods: List[Tuple[int, int]]) -> int:
        ...
        Now processes data in daily batches for incremental DB writes.

        Each day (1440 candles) is written to DB immediately after collection,
        preventing memory overflow and data loss on interruption.
        ...
        self.logger.info(f"📊 {symbol}: Collecting {len(missing_periods)} missing periods with daily batching")

        # Process each missing period using daily batching
        for period_num, (period_start, period_end) in enumerate(missing_periods, 1):
            try:
                ...
                # Split this period into daily batches
                daily_periods = self._split_period_into_days(start_date_str, end_date_str)

                period_days = (end_dt - start_dt).total_seconds() / 86400
                self.logger.info(
                    f"Period {period_num}/{len(missing_periods)}: "
                    f"{start_dt.strftime('%Y-%m-%d')} to {end_dt.strftime('%Y-%m-%d')} "
                    f"({period_days:.1f} days)"
                )

                # Create progress bar for daily batches
                pbar = tqdm(
                    daily_periods,
                    desc=f"{symbol} Days",
                    unit="day",
                ...
                # Process each day separately with immediate DB write
                for day_num, (day_start, day_end) in enumerate(pbar, 1):
                    try:
                        # Collect and save this day's data (without chunk progress bar)
                        day_candles = self.collect_symbol_data(symbol, day_start, day_end, show_progress=False)
                        total_candles_collected += day_candles

                        # Update progress bar description with current date and candles
                        day_date = day_start.split()[0]  # Extract date part
                        if day_candles > 0:
                            pbar.set_postfix({"date": day_date, "candles": day_candles, "status": "✓"})
                        else:
                            pbar.set_postfix({"date": day_date, "candles": 0, "status": "⊘"})

                    except Exception as e:
                        self.logger.error(f"Error on {day_start.split()[0]}: {e}")
                        self.errors_count += 1
                        pbar.set_postfix({"date": day_start.split()[0], "status": "✗"})
                ...
                self.logger.info(
                    f"Period {period_num} completed: {total_candles_collected:,} candles collected"
                )

            except Exception as e:
                self.logger.error(f"Error processing period {period_num}: {e}")
                self.errors_count += 1
                # Continue with next period instead of stopping
                continue

        self.logger.info(f"✅ {symbol} completed: {total_candles_collected:,} candles collected")
