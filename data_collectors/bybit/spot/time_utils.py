...
class TimeManager:
    ...
    def get_collection_period(self) -> Tuple[int, int, datetime.datetime, datetime.datetime]:
        ...
        # Limit end_date to current UTC time (don't try to collect future data)
        now_utc = datetime.datetime.now(pytz.UTC)
        if end_dt > now_utc:
            self.logger.warning(
                f"end_date ({end_dt.strftime('%Y-%m-%d %H:%M:%S')}) is in the future. "
                f"Limiting to current time ({now_utc.strftime('%Y-%m-%d %H:%M:%S')} UTC)"
            )
            end_dt = now_utc
