...
def calculate_gaps(conn, symbol: str, hours: int = 24) -> List[Tuple[str, str, int]]:
    ...
    query = """
        SELECT timestamp FROM candles_bybit_futures_1m
...
def check_data_status(verbose: bool = False, hours: int = 24):
    ...
    try:
        ...
        print("\n📊 СТАТУС ДАННЫХ FUTURES В POSTGRESQL")
        print("=" * 110)
        ...
        if symbols_with_issues:
            ...
            for symbol in symbols_with_issues:
                print(f"   ⚠️  {symbol}: Данные устарели - запустите monitor.py")
        ...
        with conn.cursor() as cursor:
            ...
            # Размер таблицы
            cursor.execute("""
                SELECT pg_total_relation_size('candles_bybit_futures_1m')
            """)
            table_size = cursor.fetchone()[0]

        print(f"\n💾 РАЗМЕР ХРАНИЛИЩА:")
        print(f"   • База данных: {db_size / 1024 / 1024 / 1024:.2f} GB")
        print(f"   • Таблица futures: {table_size / 1024 / 1024 / 1024:.2f} GB")
...
def main():
    parser = argparse.ArgumentParser(description="Проверка статуса загруженных данных futures")
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
