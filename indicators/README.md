...
- ✅ **Отслеживание времени обработки** - автоматический вывод общего времени выполнения (Total time)
...
├── obv_loader.py             # Загрузчик OBV с кумулятивным расчетом объема
...
- **OBV** (On-Balance Volume) - Балансовый объём - `obv_loader.py`
  - Автор: Joe Granville (1963), классический индикатор объемного анализа
  - 1 колонка: obv (кумулятивное значение)
  - Начальное значение: 0 (стандарт индустрии)
  - Поддерживаемые таймфреймы: 1m, 15m, 1h (все)
  - Время загрузки:
    - Полная загрузка 1m (5 лет, ~2.9M свечей): ~14-15 минут
    - Расчет OBV: ~13-16 секунд (pandas cumsum)
    - Запись в БД: ~13-14 минут (батчами по дням, ~2038 дней)
    - Ежедневное обновление (~1440 свечей): ~30 секунд
  - Особенности:
    - Кумулятивный расчет: каждое значение зависит от всех предыдущих
    - Полный пересчет: при каждом запуске OBV рассчитывается от начала истории
    - Инкрементальная запись: в БД записываются только новые данные
    - Без чекпойнтов: возобновление от MAX(timestamp) WHERE obv IS NOT NULL
    - Быстрый алгоритм: использует pandas cumsum() для эффективности
    - Автоматическое создание таблиц и колонок при первом запуске
    - Прогресс-бар с номером торговой пары: 📊 BTCUSDT [1/10] 1M
  - Формула: If Close↑ → OBV + Volume, If Close↓ → OBV - Volume, If Close= → OBV
  - Применение: дивергенции с ценой, подтверждение трендов, накопление/распределение, пробои уровней
