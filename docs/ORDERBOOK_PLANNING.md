...
**Последнее обновление:** 2026-02-04 (v3 — скрипт написан)
...
### 6. Расположение скрипта: `indicators/orderbook_loader.py`
...
```bash
python3 orderbook_loader.py                        # Обычный запуск (continue from last)
...
### Пошаговый алгоритм работы `orderbook_loader.py`
...
### Реализация: `indicators/orderbook_loader.py`
...
7. [x] Написать `indicators/orderbook_loader.py` → **580 строк, все 58 колонок, syntax verified**
8. [ ] Создать таблицу на VPS (`CREATE TABLE` встроен в скрипт, выполнится автоматически)
9. [ ] Протестировать на данных за 1 день
10. [ ] Запустить массовую загрузку исторических данных (BTCUSDT, 1,113 дней)
11. [ ] Добавить в `start_all_loaders.py` (после тестов)
12. [ ] Добавить остальные торговые пары
