...
**Реальные замеры из БД (2026-02-13, 2 снапшота, 2968 строк):**

| Метрика | На 1 снапшот | На 1 день (96) | На 1 месяц | На 1 год |
|---------|:---:|:---:|:---:|:---:|
| Строк | 1,484 | ~142K | ~4.3M | ~52M |
| Данные | 320 KB | ~30 MB | ~900 MB | ~10.7 GB |
| Индексы | 204 KB | ~20 MB | ~590 MB | ~7 GB |
| **Итого** | **540 KB** | **~50 MB** | **~1.5 GB** | **~18 GB** |

> **Примечание:** На малом объёме данных (2968 строк) индексы занимают ~64% от данных. При росте таблицы B-tree индексы становятся эффективнее — реальный overhead будет ~30-40%. Консервативная оценка: **15-18 GB/год** с учётом индексов, **~11 GB/год** только данные.

**Прогноз использования диска (при 47 GB свободно):**

| Период | Накоплено данных | Занято | Свободно |
|--------|:---:|:---:|:---:|
| Через 6 мес | ~26M строк | ~9 GB | ~38 GB |
| Через 1 год | ~52M строк | ~18 GB | ~29 GB |
| Через 2 года | ~104M строк | ~36 GB | ~11 GB |
| Через 2.5 года | ~130M строк | ~45 GB | ~2 GB ⚠️ |

> **Рекомендация:** При текущем свободном месте (47 GB) данных хватит на **~2 года** комфортного сбора. После этого потребуется: расширение диска, архивация старых данных, или уменьшение частоты снапшотов.
...
**Объём данных (реальные замеры 2026-02-13):**
- ~1,484 контрактов × 96 снапшотов/день × 365 дней = ~52M записей/год
- Данные: ~11 GB/год (~216 байт/строка), с индексами: ~15-18 GB/год (~370 байт/строка)
- При 47 GB свободно — хватит на ~2 года комфортного сбора
...
### Этап 3: Расчёт индикаторов из DVOL свечей

**Приоритет:** Высокий (сразу после Этапа 1)
**Сложность:** Средняя (отдельный скрипт-калькулятор)
**Зависимости:** Этап 1
**Статус:** ✅ Завершён (2026-02-16)

| Шаг | Действие | Статус |
|-----|----------|--------|
| 3.1 | Определить финальный список метрик для расчёта | ✅ 22 колонки в 8 группах |
| 3.2 | Создать `options_dvol_indicators_loader.py` | ✅ |
| 3.3 | Создать таблицу `options_deribit_dvol_indicators_1h` | ✅ |
| 3.4 | Рассчитать индикаторы по всей истории (BTC + ETH) | ✅ 42,874 строки × 2 валюты |
| 3.5 | Добавить в orchestrator (`start_all_loaders.py`) | ✅ |
| 3.6 | Создать документацию `docs/DVOL_REFERENCE.md` | ✅ |

**Скрипт:** `indicators/options_dvol_indicators_loader.py`
**Таблица:** `options_deribit_dvol_indicators_1h` (отдельная от DVOL свечей)
**Архитектура:** Per-group — каждая группа индикаторов обрабатывается независимо
**Документация:** `docs/DVOL_REFERENCE.md` (полный справочник с SQL-запросами)

**8 групп индикаторов (22 колонки):**

| # | Группа | Колонки | Ключ `--group` |
|---|--------|---------|----------------|
| 1 | **Trend** | dvol_sma_24, dvol_sma_168, dvol_ema_12, dvol_ema_26 | `trend` |
| 2 | **Momentum** | dvol_change_24h, dvol_change_pct_24h, dvol_roc_24h | `momentum` |
| 3 | **Levels** | dvol_percentile_30d, dvol_percentile_90d, dvol_zscore_30d | `levels` |
| 4 | **IV vs HV** | iv_hv_spread_30, iv_hv_ratio_30 | `iv_hv` |
| 5 | **BTC/ETH Cross** | dvol_btc_eth_spread, dvol_btc_eth_ratio, dvol_btc_eth_corr_24h | `cross` |
| 6 | **RSI** | dvol_rsi_14 | `rsi` |
| 7 | **Bollinger** | dvol_bb_upper_20_2, dvol_bb_lower_20_2, dvol_bb_pct_b_20_2 | `bollinger` |
| 8 | **MACD** | dvol_macd_line_12_26, dvol_macd_signal_12_26_9, dvol_macd_hist_12_26_9 | `macd` |

**Использование:**
```bash
python3 options_dvol_indicators_loader.py                              # Все группы, BTC + ETH
python3 options_dvol_indicators_loader.py --group rsi                  # Только RSI
...
**Результаты загрузки (2026-02-16):**

| Группа | BTC | ETH | Покрытие |
|--------|-----|-----|----------|
| Trend | 42,851 | 42,851 | 99.9% |
| Momentum | 42,850 | 42,850 | 99.9% |
| Levels | 42,707 | 42,707 | 99.6% |
| IV vs HV | 42,854 | 42,854 | 99.9% |
| BTC/ETH Cross | 42,874 | 42,874 | 100% |
| RSI | 42,859 | 42,859 | 99.9% |
| Bollinger | 42,855 | 42,855 | 99.9% |
| MACD | 42,874 | 42,874 | 100% |

> Пропуски (0.1-0.4%) — нормальные NULL в начале данных из-за warm-up периодов индикаторов (SMA 168 = 168 свечей, percentile 90d = 2160 свечей и т.д.).
...
| Период | Строк | Данные | С индексами |
|--------|:---:|:---:|:---:|
| 1 снапшот (15 мин) | ~1,484 | 320 KB | 540 KB |
| 1 день (96 снапшотов) | ~142K | ~30 MB | ~50 MB |
| 1 месяц | ~4.3M | ~900 MB | ~1.5 GB |
| 1 год | ~52M | ~11 GB | ~15-18 GB |

При 47 GB свободно на VPS — хватит на ~2 года сбора.
...
- Таблица: `options_deribit_aggregated_15m` (одна строка на timestamp × currency)
- Per-group архитектура: 7 групп метрик, каждая обрабатывается независимо
- Инкрементальная загрузка: каждая группа проверяет свой последний заполненный timestamp
- Daily batching: UPSERT только колонки группы, commit после каждого дня

| Шаг | Действие | Статус |
|-----|----------|--------|
| 5.1 | Создать `options_aggregated_loader.py` (7 групп, 24 колонки) | ✅ |
| 5.2 | Создать таблицу `options_deribit_aggregated_15m` | ✅ |
| 5.3 | Загрузить данные BTC + ETH | ✅ |
| 5.4 | Добавить в orchestrator (`start_all_loaders.py`) | ✅ |
| 5.5 | Обновить документацию (`DVOL_REFERENCE.md`, `CLAUDE.md`) | ✅ |
| 5.6 | Интеграция с ML pipeline | |

**7 групп метрик (24 колонки):**

| # | Группа | Колонки | Ключ `--group` |
|---|--------|---------|----------------|
| 1 | **Volume/OI** | put_call_ratio_volume/oi, total_volume_24h, total_open_interest, oi_change_pct_24h | `volume` |
| 2 | **IV Metrics** | iv_atm_30d, iv_25d_put/call_30d, iv_skew_25d_30d, iv_smile_steepness_30d, iv_term_structure_7d_30d | `iv` |
| 3 | **Max Pain** | max_pain_nearest/monthly + distance_pct | `maxpain` |
| 4 | **Greeks Exposure** | gex, net_delta, net_gamma, vega_exposure | `greeks` |
| 5 | **Expiration** | days_to_expiry_nearest, notional_expiring_7d | `expiry` |
| 6 | **Liquidity** | bid_ask_spread_avg_atm, max_oi_strike | `liquidity` |
| 7 | **Positioning** | gamma_flip_level | `positioning` |

**Использование:**
```bash
python3 options_aggregated_loader.py                                # Все группы, BTC + ETH
python3 options_aggregated_loader.py --group iv --currency BTC      # Только IV для BTC
...
| Группа | BTC | ETH | Покрытие |
|--------|-----|-----|----------|
| Volume/OI | 301 | 301 | 100% |
| IV Metrics | 301 | 301 | 100% |
| Max Pain | 301 | 301 | 100% |
| Greeks Exposure | 301 | 301 | 100% |
| Expiration | 301 | 301 | 100% |
| Liquidity | 301 | 301 | 100% |
| Positioning | 301 | 301 | 100% |

> Данные за 4 дня (2026-02-13 — 2026-02-16), 301 строка на валюту (96 снапшотов/день × 15 мин).

**Примеры значений BTC (2026-02-16):**

| Метрика | Значение | Интерпретация |
|---------|----------|---------------|
| Put/Call Ratio (volume) | 0.35 | Жадность (больше покупают коллов) |
| IV ATM 30d | 49.4% | Нормальная волатильность |
| IV Skew 25d | +5.5 п.п. | Умеренный страх падения |
| Max Pain (nearest) | $69,000 | Ценовой магнит ближайшей экспирации |
| GEX | -10.6M | Negative gamma (усиление движений) |
| Gamma Flip Level | $70,577 | Выше — стабилизация, ниже — хаос |

**Баг-фикс gamma_flip_level (2026-02-16):**
- Исходная реализация умножала ВСЕ gamma×OI на test_price² и суммировала → знак net GEX не менялся → всегда NULL
- Исправлено: per-strike GEX вклад каждого страйка отдельно, поиск ближайшей к цене смены знака
- Результат: 100% покрытие (было 0%)

#### Метрики, требующие отдельного решения

##### OI по страйкам (распределение, не скаляр)

Аналитик запросил "OI по страйкам" — это **не одно число**, а распределение Open Interest по всем страйк-ценам. Для BTC это ~85 страйков (от 20K до 380K), для ETH — аналогично. Это принципиально отличается от остальных 16 метрик, каждая из которых — одно число на timestamp.

**Почему нельзя хранить как обычную колонку:**
- Количество страйков меняется (сейчас 85, завтра 90)
- Страйк-цены разные для каждой экспирации
- Нужна группировка по экспирациям (OI для ближайшей vs квартальной)

**Варианты решения:**

| Вариант | Хранение | Плюсы | Минусы |
|---------|----------|-------|--------|
| JSONB колонка | `oi_by_strike JSONB` в aggregated таблице | Компактно, один JOIN | Сложные запросы |
| Скалярные метрики | `max_oi_strike`, `max_oi_value`, `oi_concentration_top5` | Просто, стандартные колонки | Теряем распределение |
| Отдельная таблица | `options_oi_by_strike_15m (timestamp, currency, expiration, strike, oi)` | Полная детализация | Много строк (~85 × 12 exp × 96/day = ~98K строк/день) |

**Рекомендация:** Комбинированный подход:
1. **Скалярные метрики** в aggregated таблице: `max_oi_strike`, `max_oi_value`, `oi_concentration_top5_pct`
2. **JSONB** для топ-10 страйков: `oi_top_strikes JSONB` — `[{"strike": 100000, "oi_call": 5000, "oi_put": 3000}, ...]`
3. Полная детализация — через SQL запрос к `options_deribit_raw` напрямую (данные уже там)

Решение принимается при реализации Этапа 5.

##### Large trades / Block trades (нет данных)

Аналитик запросил "Large trades/blocks" — крупные сделки на рынке опционов (>1 BTC), которые показывают направление smart money.

**Текущая ситуация: данных нет и мы их не собираем.**

Наш WebSocket collector подписан на `ticker.{instrument}.100ms`, который возвращает агрегированные данные (OI, volume, IV, Greeks) — но **не отдельные сделки**. Для получения сделок нужен отдельный канал:

```
WebSocket: trades.{instrument_name}.raw     ← отдельные сделки в реальном времени
REST:      public/get_last_trades_by_currency ← последние сделки (ограниченная история)
```
...
|-----|----------|-----------|
| 1 | Создать таблицу `options_deribit_trades` | Низкая |
| 2 | Добавить подписку `trades.*.raw` в WebSocket collector | Средняя |
| 3 | Фильтровать large trades (>1 BTC / >10 ETH) | Низкая |
| 4 | Агрегировать в `large_trade_flow` метрику | Низкая |

**Объём данных (оценка):**
- ~500-2000 сделок/день для BTC опционов (Deribit)
- Large trades (>1 BTC): ~50-200/день
- Хранение: ~1-5 MB/день, ~0.5-2 GB/год

**Решение:** Отложить до завершения Этапа 5. Добавить сбор trades как Этап 6 — отдельная доработка WebSocket collector + отдельная таблица. Базовые 14 метрик из Этапа 5 значительно ценнее и не требуют дополнительного сбора данных.

### Сравнение этапов и приоритеты

| | Этап 1-2 (DVOL свечи) | Этап 3 (DVOL индикаторы) | Этап 4 (WebSocket сбор) | Этап 5 (агрегация) |
|--|----------------------|------------------------|------------------------|-------------------|
| Статус | ✅ Завершён | ✅ Завершён | ✅ Завершён | ✅ Завершён |
| Данные | 5 лет DVOL OHLC | 22 индикатора из DVOL | Raw опционы (накапливаются) | 24 метрики из raw |
| Время до результата | Готово | Готово | Мгновенно (raw), 1-3 мес для ML | Готово |
| Ценность для ML | Базовые данные | Высокая (iv_hv_spread, RSI, BB, MACD) | Нет (сырые данные) | Очень высокая (14/17 метрик аналитика) |
| Хранение | 12 MB (1h) + 73 MB (1m) | 28 MB | ~11 GB/год | ~10 MB/год |
| Метрик аналитика | — | 2/17 (+ 20 доп. индикаторов) | — | 14/17 |

**Все основные этапы завершены.** Оставшиеся задачи: Этап 6 (сбор trades для large_trade_flow), интеграция с ML pipeline.
