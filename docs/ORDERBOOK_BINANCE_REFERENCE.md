...
Фаза 2 — Monthly bookTicker fallback (только с --check-nulls):
  1. Найти месяцы с NULL bookTicker (начиная с BOOK_TICKER_EARLIEST)
  2. Запросить конкретные NULL-даты из БД для каждого месяца
  3. Скачать monthly ZIP на диск (/tmp) с прогресс-баром (~3-4 GB)
  4. Распаковать CSV на диск, удалить ZIP
  5. Chunked чтение CSV с диска (по 5M строк), фильтр только NULL-дат
  6. Накопление данных per-day → агрегация → UPDATE 22 bookTicker колонки
  7. Удалить CSV. Память: ~300 MB (чанки, ZIP/CSV на диске)
```
...
| Строк/день | 1,440 |
| Текущие данные | BTCUSDT: 1,611,571 строк, 550 MB |
| bookDepth покрытие | 100% (все строки) |
| bookTicker покрытие | 28.5% (459,515 строк, 2023-05-16 → 2024-04-01) |
| Скрипт | `indicators/orderbook_binance_loader.py` |
...
| Флаг | Описание |
|------|----------|
| `--symbol BTCUSDT` | Обработать конкретный символ (по умолчанию: все из конфига) |
| `--force-reload` | Перезагрузить все данные с 2023-01-01 |
| `--check-nulls` | Найти дни с NULL bookDepth + запустить monthly bookTicker fallback |
...
| 2024-03-31 | 100% (из monthly) | Заполнено из `BTCUSDT-bookTicker-2024-03.zip` |
| 2024-04-01 | 28% (407 строк, до 06:46 UTC) | Из `BTCUSDT-bookTicker-2024-04.zip` |
| 2024-04-02+ | 0% | Binance прекратил публикацию (daily + monthly) |

### Типичные значения (проверено на 1.6M строк)
...
### Текущий размер и прогноз

| Конфигурация | Размер |
|---|---|
| BTCUSDT (3.1 года) | 550 MB (1.6M строк) |
| 1 символ / год | ~0.17 GB |
| 17 символов / год | ~2.9 GB |
| 17 символов × 3 года | ~8.7 GB |
...
### 3. OOM и некорректная агрегация monthly bookTicker

**Проблема 1 (OOM):** Monthly ZIP файлы (~3-4 GB, 135M строк) вызывали OOM kill на VPS при загрузке в один DataFrame.

**Проблема 2 (дубли):** CSV данные в monthly архивах не отсортированы хронологически. Потоковая обработка по границам дней приводила к многократной частичной обработке одного дня, каждый UPDATE перезаписывал предыдущий неполный результат.

**Проблема 3 (лишние месяцы):** Fallback скачивал ВСЕ месяцы с NULL bookTicker, включая те, для которых архивов не существует (2024-05+), тратя время на 404 запросы.

**Исправление:**
1. Chunked чтение CSV: `pd.read_csv(chunksize=5_000_000)` с tqdm прогресс-баром
2. Запрос конкретных NULL-дат из БД (`_get_null_ticker_dates()`) — фильтрация при чтении
3. Накопление ВСЕХ чанков per-day в `dict {day: [chunks]}`, затем обработка целиком
4. Прогресс-бар для скачивания (`stream=True` + `Content-Length`)
5. Fallback ищет с `BOOK_TICKER_EARLIEST` (покрывает все месяцы, включая март 2024)

**Результат:** Корректная агрегация (каждый день обрабатывается один раз), низкое потребление RAM (только данные для NULL-дат).

**Статус:** Исправлено.

### 4. Пропущенные и неполные дни

5 дней с минимальными данными (< 100 строк из ~1440):
- 2023-01-09 (4), 2023-01-15 (3), 2023-01-16 (5), 2023-01-21 (2), 2024-06-12 (1)

1 день полностью отсутствует: 2024-04-18 (нет bookDepth файла в архиве Binance).

Это нормально для публичных архивов Binance.
