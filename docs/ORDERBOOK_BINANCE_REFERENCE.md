...
Фаза 2 — Monthly bookTicker fallback:
  ...
  3. Скачать monthly ZIP с прогресс-баром (~3-4 GB)
  4. Chunked чтение CSV (по 5M строк), фильтр только NULL-дат
  5. Накопление данных per-day → агрегация → UPDATE 22 bookTicker колонки
  6. Память: только данные для NULL-дат (~1-2 GB пик)
  7. Автоматически, при каждом запуске (пропускается если данные уже заполнены)
```
...
| `--check-nulls` | Найти дни с NULL в bookDepth колонках и перезагрузить только их |
