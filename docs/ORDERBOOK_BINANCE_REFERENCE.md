...
Фаза 2 — Monthly bookTicker fallback:
  1. Найти месяцы, где bookDepth есть но bookTicker = NULL (начиная с 2024-04-01)
  2. Скачать monthly ZIP (напр. BTCUSDT-bookTicker-2024-04.zip, ~3-4 GB)
  3. Chunked чтение CSV (по 5M строк) → обработка по дням → UPDATE 22 bookTicker колонки
  4. Память: ~1-2 GB пик (безопасно для 16 GB VPS)
  5. Автоматически, при каждом запуске (пропускается если данные уже заполнены)
```
...
### 3. OOM при обработке monthly bookTicker

**Проблема:** Monthly ZIP файлы (~3-4 GB, 135M строк) вызывали OOM kill на VPS с 16 GB RAM при загрузке в один DataFrame.

**Дополнительный баг:** SQL запрос `timestamp > BOOK_TICKER_LAST` (2024-03-30 00:00) включал 31 марта, что запускало ненужную загрузку мартовского архива.

**Исправление:**
1. `BOOK_TICKER_MONTHLY_START = 2024-04-01` — пропуск марта 2024
2. Chunked чтение CSV: `pd.read_csv(chunksize=5_000_000)` (~1 день данных за чанк)
3. По-дневная обработка: накопление чанков → обработка полного дня → UPDATE в БД → освобождение памяти
4. Извлечение `_aggregate_ticker_df()` — общая логика для daily и monthly путей

**Результат:** Пик RAM ~1-2 GB (было 16+ GB), безопасно для 16 GB VPS.
